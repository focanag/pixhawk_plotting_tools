...
def alt_analysis(data, min_alt=None, max_alt=None):
    ...
    Altitude analysis.
    """
    #not working for some reason (weird)
    #i0_Baro = data.SENS_BaroAlt.first_valid_index()
    #i0_GPS = data.GPS_Alt.first_valid_index()
    #it prints i0_Baro = 0.0 and i0_GPS = 0.567
    
    #to avoid nan values
    index1 = 0
    for d in data.SENS_BaroAlt.values:
      if math.isnan(d):
          index1 = index1 + 1

    #to avoid nan values
    index2 = 0
    for d in data.GPS_Alt.values:
      if math.isnan(d):
          index2 = index2 + 1

    #if one have values before the other and we want the values of both at the same time
    index = 0 
    if index2 >= index1:
        index = index2
    else:
    ...
    (data.SENS_BaroAlt - data.SENS_BaroAlt.values[index]).plot(legend=True)
    (data.GPS_Alt - data.GPS_Alt.values[index]).plot(legend=True)
